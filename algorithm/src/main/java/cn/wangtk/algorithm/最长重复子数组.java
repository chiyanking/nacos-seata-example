...
public class 最长重复子数组 {
    ...
    public static void main(String[] args) {
        int[] s1 = new int[]{1, 2, 3, 2, 1};
        int[] s2 = new int[]{3, 2, 1, 4, 7};
        ...
        for (int i = 0; i < s1.length; i++) {
            for (int j = 0; j < s2.length; j++) {
                ...
                System.out.println(Arrays.toString(Arrays.copyOfRange(s1, 0, i + 1)) + " : "
                        + Arrays.toString(Arrays.copyOfRange(s2, 0, j + 1)) + " : " + memo[i + 1][j + 1]);
            }
        }
